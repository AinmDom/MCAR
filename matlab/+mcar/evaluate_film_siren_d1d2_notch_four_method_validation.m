function evaluate_film_siren_d1d2_notch_four_method_validation( ...
        ...
        ranfPredictionName, fspPredictionName, primaryMethodId, ...
        primaryMethodLabel)
...
if nargin < 7 || isempty(primaryMethodId)
    primaryMethodId = 'FILM';
end
if nargin < 8 || isempty(primaryMethodLabel)
    primaryMethodLabel = 'FiLM-SIREN D1/D2+notch E130 1/3 ensemble';
end
...
validateattributes(primaryMethodId, {'char', 'string'}, {'scalartext'});
validateattributes(primaryMethodLabel, {'char', 'string'}, {'scalartext'});
...
primaryMethodId = string(primaryMethodId);
primaryMethodLabel = string(primaryMethodLabel);
assert(isvarname(primaryMethodId), 'Primary method ID must be a valid variable name.');
...
methodIds = [primaryMethodId, "MCAR", "RANF", "FSPAE"];
methodLabels = [primaryMethodLabel, ...
...
for subjectIndex = 1:subjectCount
    ...
    % MATLAB h5read returns dimensions in reversed order, so the FSP-AE
    % HRIR [793, 2, 256] (direction, ear, time) is [256, 2, 793] here.
    fspHrirRaw = double(h5read(fspFile, '/predicted_hrir'));
    assert(isequal(size(fspHrirRaw), [hrirLength, 2, 793]), ...
    ...
    row = table(subjectLabel, subjectNumber, ...
        ...
        'VariableNames', [{'SubjectLabel', 'SubjectID'}, cellstr([ ...
        methodIds + "FullSphereERB_dB", ...
        methodIds + "Contralateral25ERB_dB", ...
        methodIds + "ContralateralHighFrequency_dB", ...
        methodIds + "HorizontalILDMAE_dB"])]);
...
summary = struct('schema_version', '1.0', 'status', 'completed', ...
    ...
    'reconstruction', ...
        'Primary/MCAR: predicted magnitude with original MCA phase and outside bins; RANF: SOFA HRIR; FSP-AE: predicted HRIR', ...
...
for metricIndex = 1:numel(metricIds)
    ...
    row = table(metricIds(metricIndex), metricNames(metricIndex), ...
        ...
        cellstr(reshape([methodIds + "Mean_dB"; methodIds + "Std_dB"], 1, []))]);
...
barData = zeros(height(aggregate), numel(methodIds));
for methodIndex = 1:numel(methodIds)
    barData(:, methodIndex) = aggregate.(methodIds(methodIndex) + "Mean_dB");
end
