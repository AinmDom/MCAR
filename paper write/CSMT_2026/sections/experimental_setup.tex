...
主比较包含七个外部方法与本文Hybrid：SH only、SUpDEq SH、SUpDEq Natural Neighbor、SUpDEq Barycentric、MCA、FSP-AE和RANF构成外部方法集合。结果叙述首先检验相对MCA的残差补偿效果，再比较外部学习方法。项目推进中的内部中间模型仅用于第\ref{sec:ablation}节的消融与内部变体分析，不计入横向基线。
...
对Hybrid等只输出MCA幅度残差的方法，严格评价使用第\ref{sec:method}节的MCA相位及未预测频点复用链路。对直接生成HRIR的方法，保留其原生输出并映射至统一表示和评价频点，不强制替换为MCA相位。补充评价的统一导出需复现既有四项主要指标，接受阈值为逐被试最大绝对差不超过$10^{-9}$\,dB。包含内部变体的既有完整性归档覆盖10个配置、44名被试和20端点，共8800个有限逐被试值，主要指标来源复核误差为0。本文从该归档中分开呈现八方法主比较与内部变体分析，不重新生成测试预测，也不将项目复现配置等同于各原论文的全部设置。
...
\end{equation}
为配对差值；负值代表Hybrid误差较小。外部方法的直接配对比较包含MCA、FSP-AE和RANF；内部变体的既有配对结果仅在第\ref{sec:ablation}节报告。方向和频点不作为独立被试扩充样本量，三个训练成员也不作为三个独立测试样本。测试被试全部保留，不按误差剔除离群个体。

补充端点既有统计使用种子20260902及其确定性派生种子；归档中的补充配对表并非以Hybrid为中心，不能重新标记成Hybrid中心的显著性结果。本文对缺少直接Hybrid配对区间的端点只报告描述性位置。分析没有多重比较校正，也没有预先设定统计等效界限，区间含零不解释为等效，接近的均值亦不直接合并为“并列最优”。
...
观测方向敏感性使用嵌套的$Q14\subset Q26\subset Q50$，并统一在排除全部Q50观测后的743个方向上评价，只使用44名验证被试。本文重点报告Hybrid的冻结参数响应。已有外部方法对照中，FSP-AE保持网络参数冻结，传统方法按当前观测集重建；RANF遵循其原生路径，从同一冻结预训练参数出发，在Q14与Q50分别进行1000个epoch、batch size为3的适配，Q26复用既有适配产物。因此，不将不同方法各自既定路径的输入敏感性概括为全部模型均不适配的统一实验。
