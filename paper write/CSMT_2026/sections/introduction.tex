...
% Role I-P4: research questions, preserved verbatim from the approved framework.
本文围绕三个问题展开：Hybrid相对MCA、MCAR、Bounded、FSP-AE、RANF的优势与代价是什么？在冻结FiLM-SIREN上追加CNN后，哪些误差发生变化，结构与训练开销如何界定？Q26训练并冻结的Hybrid对Q14/Q50观测如何响应？这些问题分别对应历史测试队列比较、验证集组件比较和输入方向敏感性分析。
...
SONICOM历史测试队列的结果显示，Hybrid相对MCA降低了三项主要频谱误差，并在对侧ERB及三项局部谱结构指标上取得所比较方法中的最低均值。然而，相对MCAR的水平面耳间声级差误差增大，输入方向增加也未带来一致收益。本文据此将Hybrid定位为具有特定频谱收益的残差细化方案，不由客观指标推断听感改善。第\ref{sec:setup}节说明历史测试访问与事后写作选择，第\ref{sec:results}节报告完整比较及其限制。
