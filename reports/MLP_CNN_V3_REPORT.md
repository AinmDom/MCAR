# 幅度校正与时间对齐插值（Magnitude-Corrected and Time-Aligned Interpolation，MCA）Residual 多层感知机-卷积神经网络（Multi-Layer Perceptron–Convolutional Neural Network，MLP-CNN）v3 完整设计与实验报告

缩写说明：头相关传输函数（Head-Related Transfer Function，HRTF）、头相关脉冲
响应（Head-Related Impulse Response，HRIR）、特征级线性调制（Feature-wise
Linear Modulation，FiLM）、等效矩形带宽（Equivalent Rectangular Bandwidth，
ERB）、耳间电平差（Interaural Level Difference，ILD）、平均绝对误差（Mean
Absolute Error，MAE）、均方根误差（Root Mean Squared Error，RMSE）、16 位
浮点数（16-bit Floating Point，FP16）、自动混合精度（Automatic Mixed
Precision，AMP）、Weights & Biases（W&B）、球谐函数（Spherical Harmonics，
SH）、耳间时间差（Interaural Time Difference，ITD）、空间声学数据格式
（Spatially Oriented Format for Acoustics，SOFA）、Hierarchical Data Format
version 5（HDF5）、快速傅里叶变换（Fast Fourier Transform，FFT）、快速
傅里叶逆变换（Inverse Fast Fourier Transform，IFFT）、图形处理器（Graphics
Processing Unit，GPU）、统一计算设备架构（Compute Unified Device
Architecture，CUDA）、命令行界面（Command-Line Interface，CLI）、Sigmoid
线性单元（Sigmoid Linear Unit，SiLU）、高频（High Frequency，HF）、统一资源
定位符（Uniform Resource Locator，URL）、标识符（Identifier，ID）、逗号分隔值
...
> 见 `../docs/PROJECT_STRUCTURE.md` 与 `../experiments/residual_mlp_cnn/`。
...
生成图首先写入 `artifacts/`。用于论文和交接的精选图位于
`results/residual_mlp_cnn/mlp_cnn_n03_v3/evaluation/figures/`。
...
```text
src/mcar/
├── models/residual_mlp_cnn.py
├── training/train_mlp_cnn_v3.py
└── evaluation/
...
`matlab/+mcar/evaluate_test_reconstruction.m` 增加可选：
...
```powershell
D:\miniconda3\envs\ml\python.exe -m pip install -e ".[tracking]"
```

### 21.2 Smoke test

```powershell
D:\miniconda3\envs\ml\python.exe tests/smoke_test_mlp_cnn.py `
  --dataset-root data/processed/hutubs_residual_v1_n03 `
  --checkpoint artifacts/training/mlp_n03_v2/best.pt `
...
```powershell
D:\miniconda3\envs\ml\python.exe -m mcar.evaluation.predict_mlp_cnn_reconstruction `
...
```powershell
matlab.exe -batch `
  "addpath('matlab'); mcar.evaluate_mlp_cnn_v3_reconstruction; mcar.plot_v1_v2_v3_reconstruction_comparison"
```
