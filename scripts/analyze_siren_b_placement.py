"""Aggregate Stage B placement screening and Top-2 seed follow-up."""
...
def run_name(placement: str, seed: int) -> str:
    return f"sonicom_film_siren_b_placement_{placement}_seed{seed}"
...
def read_run(root: Path, placement: str, seed: int) -> dict[str, object] | None:
    ...
    if report["test_subjects_read"] != 0 or bool(report["git"]["dirty"]):
...
def analyze(root: Path) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    screening_complete = len(screening) == len(PLACEMENTS)
    top2 = [str(row["placement"]) for row in screening[:2]] if screening_complete else []
    screening_ranking = [
        {
            "rank": rank,
    ...
    ranking_reversed = bool(ranking) and str(ranking[0]["placement"]) != top2[0]
