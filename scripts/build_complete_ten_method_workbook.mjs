...
const resultDir = path.join(root, "results", "sonicom_complete_ten_method_comparison_v1");
const outputDir = path.join(root, "outputs", "stage_e_complete_ten_method_comparison");
...
overview.getRange("B5:B8").values = [[10], [9], [5], [5]];
overview.getRange("C5:C8").values = [[10], [10], [10], [10]];
overview.getRange("D5:D8").values = [["44-subject validation"], ["Historical test; Hybrid E190 NOT RUN"], ["Supplementary validation"], ["Exploratory validation"]];
...
chart.title = "Full-sphere ERB: validation and historical frozen test";
...
await output.save(path.join(outputDir, "stage_e_complete_ten_method_comparison.xlsx"));
console.log(JSON.stringify({ sheetNames, formulaErrors, output: path.join(outputDir, "stage_e_complete_ten_method_comparison.xlsx") }));
