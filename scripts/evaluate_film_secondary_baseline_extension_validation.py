"""Extend frozen FiLM secondary metrics to RANF and FSP-AE on validation only."""
...
METHODS = ("RANF", "FSPAE")
METHOD_LABELS = {"RANF": "RANF", "FSPAE": "FSP-AE"}
SCALAR_UNITS = {
    "FullSphereLSD": "dB",
...
def main() -> None:
    ...
    for method, spec in method_specs.items():
        ...
        if inventory_digest(
            prediction_root, subject_labels, spec["filename"]
    ...
    max_ranf_coordinate_error = 0.0
    max_fsp_frequency_error_hz = 0.0
    ...
    for subject_index, (subject_id, subject_label, source_path) in enumerate(
        ...
        ranf_path = method_roots["RANF"] / "subjects" / subject_label / method_specs[
            "RANF"
        ]["filename"]
        predictions["RANF"] = load_ranf_prediction(
            ranf_path, selected_indices, directions, sampling_rate
        )
        with h5py.File(ranf_path, "r") as handle:
            ranf_position = np.asarray(handle["SourcePosition"][:], dtype=np.float64)
        ranf_az = np.abs(
            np.mod(ranf_position[:, 0] - directions[:, 0] + 180.0, 360.0) - 180.0
        )
        max_ranf_coordinate_error = max(
            max_ranf_coordinate_error,
            float(max(ranf_az.max(), np.abs(ranf_position[:, 1] - directions[:, 1]).max())),
        )
        fsp_path = method_roots["FSPAE"] / "subjects" / subject_label / method_specs[
            "FSPAE"
        ]["filename"]
        predictions["FSPAE"] = load_fspae_prediction(
            fsp_path, selected_indices, frequency, subject_label
        )
        with h5py.File(fsp_path, "r") as handle:
            fsp_frequency = np.asarray(handle["frequency_hz"][:], dtype=np.float64)
        max_fsp_frequency_error_hz = max(
            max_fsp_frequency_error_hz,
            float(np.max(np.abs(fsp_frequency[selected_indices - 1] - frequency))),
        )
    ...
    comparator_primary = load_metric_rows(
        ...
        value_column="Value_dB",
        endpoint_column="Metric",
        accepted_methods={"RANF": "RANF", "FSPAE": "FSPAE"},
    )
    ...
    for endpoint, methods in tail_values.items():
        if set(methods) != {"BOUNDED", "RANF", "FSPAE"}:
    ...
    quality = {
        ...
        "max_ranf_coordinate_error_degrees": max_ranf_coordinate_error,
        "max_fsp_frequency_error_hz": max_fsp_frequency_error_hz,
        "row_counts": {
