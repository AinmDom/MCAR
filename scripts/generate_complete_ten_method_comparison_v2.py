...
OUTPUT = ROOT / "results" / "sonicom_complete_ten_method_comparison_v2"
METHODS = (
    "SHOnly",
    "SUpDEqSH",
    "SUpDEqNN",
...
def canonical_subject_rows() -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    test_source = (
        "results/sonicom_film_siren_spectral_cnn_final_e190_"
        "frozen_test_ten_method/metric_long.csv"
    )
    test = []
...
def long_mean_rows(
    ...
    tiers = (
        ...
        ("Primary frozen engineering test", "test", PRIMARY_ENDPOINTS, test, set(METHODS), "Frozen engineering test"),
        ("Secondary validation", "val", SECONDARY_ENDPOINTS, supplementary, set(METHODS), "Validation supplementary"),
        ("Deferred validation", "val", DEFERRED_ENDPOINTS, supplementary, set(METHODS), "Exploratory validation"),
    )
    rows = []
    for tier, split, endpoints, values, available, evidence in tiers:
        for endpoint in endpoints:
            ...
            for method in METHODS:
                ...
                reason = ""
                if not is_available:
                    reason = "No frozen same-protocol reconstruction/result for this endpoint tier"
                rows.append(
                    {
                        "EvidenceTier": tier,
                        "Split": split,
...
def availability_rows() -> list[dict[str, object]]:
    available_secondary = set(METHODS)
    groups = (
        ("Primary validation", set(METHODS), "AVAILABLE", "44-subject strict validation"),
        ("Primary frozen engineering test", set(METHODS), "NOT RUN", "No frozen engineering-test result"),
...
def profile_rows(filename: str) -> list[dict[str, object]]:
    ...
    sources = (
        ...
        ("results/sonicom_film_secondary_classical_baseline_extension_v1_validation", {method: method for method in METHODS[:5]}),
    )
...
def main() -> None:
    ...
    validate_subject_rows(test_rows, METHODS)
    ...
    pairs.extend(paired_rows("Primary frozen engineering test", "test", test_rows, PRIMARY_ENDPOINTS, METHODS))
    pairs.extend(paired_rows("Secondary validation", "val", secondary_subjects, SECONDARY_ENDPOINTS, METHODS))
    paired_deferred_endpoints = tuple(e for e in DEFERRED_ENDPOINTS if e != "ReferenceNotchFraction")
    pairs.extend(paired_rows("Deferred validation", "val", deferred_subjects, paired_deferred_endpoints, METHODS))
    ...
    if len(means) != 240 or len(numeric_means) != 240 or not np.all(np.isfinite(numeric_means)):
        raise ValueError("Mean-table completeness check failed")
    if len(pairs) != 9 * 4 + 9 * 4 + 9 * 9 + 9 * 6:
    ...
    summary = {
        ...
        "primary_test_method_count": 10,
        "primary_test_missing": [],
        "secondary_validation_method_count": 10,
        "deferred_validation_method_count": 10,
    ...
    lines = [
        ...
        "- Frozen engineering test: 10/10 methods, including the unique Hybrid E190 result.",
        "- Secondary and deferred validation: 10/10 methods with frozen same-protocol evidence.",
