"""Freeze the validation-only five-classical-baseline extension manifest."""
...
def inventory_digest(root: Path, subjects: list[str], method: str, filename: str) -> str:
    digest = hashlib.sha256()
...
def main() -> None:
    ...
    unexpected_tracked = [line for line in tracked if line[3:] != ".gitignore"]
    if unexpected_tracked:
        raise RuntimeError(
            f"Unexpected tracked modifications before freeze: {unexpected_tracked}"
        )
    head = subprocess.run(
    ...
    prediction_relative = (
        "artifacts/reconstruction/sonicom_classical_secondary_baselines_v1_validation"
    )
    prediction_root = root / prediction_relative
    provenance_relative = (
        "artifacts/evaluation/sonicom_classical_secondary_export_v1/summary.json"
    )
    ...
    output_relative = (
        "results/sonicom_film_secondary_classical_baseline_extension_v1_validation"
    )
    ...
    payload = {
        ...
        "purpose": "Validation-only extension of frozen FiLM secondary/deferred metrics to five classical baselines",
        "protocol": resource(
            root,
            "experiments/film_siren/FUTURE_FILM_SECONDARY_CLASSICAL_BASELINE_EXTENSION_AMENDMENT_V1.md",
        ),
        ...
        "methods": {
            method: {
                "label": label,
                "prediction_root": prediction_relative,
                "filename": "prediction.h5",
                "prediction_inventory_sha256": inventory_digest(
                    prediction_root, labels, method, "prediction.h5"
                ),
                "provenance_path": provenance_relative,
                "provenance_sha256": file_sha256(root / provenance_relative),
                "spectral_mapping": "direct frozen 463-bin magnitude and 256-sample HRIR export",
            }
            for method, label in (
                ("SHOnly", "SH only"),
                ("SUpDEqSH", "SUpDEq SH"),
        ...
        "existing_results": {
            ...
            "comparator_primary": resource(
                root, "results/sonicom_fsp_ae_q26_formal_validation/metric_long.csv"
            ),
        ...
        "implementation": {
            "git_commit": head,
            "git_dirty_at_freeze": bool(tracked),
            "excluded_user_owned_tracked_changes": [".gitignore"] if tracked else [],
            ...
            "resources": [
                resource(root, "scripts/evaluate_film_secondary_classical_baselines_validation.py"),
                resource(root, "matlab/+mcar/export_sonicom_classical_secondary_validation.m"),
                resource(root, "matlab/+mcar/evaluate_sonicom_interpolation_baselines.m"),
        ...
        "statistics": {
            ...
            "paired_difference": "BOUNDED minus classical baseline; negative favors BOUNDED",
        },
        "availability": {
        ...
        "command": "D:\\miniconda3\\envs\\ml\\python.exe scripts/evaluate_film_secondary_classical_baselines_validation.py configs/experiments/sonicom_film_secondary_classical_baselines_v1_validation_manifest.json",
        "test_policy": "Validation only; no test path may be constructed; reports record test_subject_count_read=0.",
        "result_blind_statement": "Endpoint formulas and existing candidate/RANF/FSP-AE results were already known. No secondary/deferred output for these five classical baselines was computed, aggregated, or inspected before this comparator-specific freeze. Previously committed classical primary metrics are used only for preregistered paired tail-risk rows.",
    }
    encoded = json.dumps(
        payload, sort_keys=True, separators=(",", ":"), ensure_ascii=True
    ).encode("utf-8")
    payload["identity_sha256"] = hashlib.sha256(encoded).hexdigest().upper()
    destination = root / "configs/experiments/sonicom_film_secondary_classical_baselines_v1_validation_manifest.json"
