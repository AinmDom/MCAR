"""Generate seed follow-ups for the frozen Stage B modulation Top-2."""
...
def main() -> None:
    root = project_root()
    decision_path = (
        root / "results" / "sonicom_film_siren_b_modulation" / "decision.json"
    )
    ...
    if (
        ...
        raise ValueError(f"Modulation Top-2 is not frozen: {decision}")
    outputs: list[str] = []
    for variant in top2:
        base_path = (
            root
        ...
        for seed in SEEDS:
            ...
            configuration["experiment_id"] = (
                f"SIREN-B-MODULATION-{variant.upper()}-SEED-{seed}"
            )
            configuration["model_version"] = (
                f"film-siren-b-modulation-{variant}-seed-{seed}-v1"
            )
