"""Run the predeclared 10-epoch v3.2.1 joint MLP+CNN fine-tune."""
...
CONFIG_PATH = (
    ...
    / "configs"
    / "experiments"
    / "sonicom_mlp_cnn_q26_v321_joint_unfreeze_e10.json"
)
OUTPUT_DIR = (
    ROOT
    / "artifacts"
    / "training"
    / "sonicom_mlp_cnn_q26_v321_joint_unfreeze_e10"
)
...
def main() -> None:
    ...
    if not config["training"]["unfreeze_mlp"]:
        raise RuntimeError("v3.2.1 must explicitly unfreeze the MLP")
    ...
    if _complete(config):
        print("Completed v3.2.1 run found; skipping.", flush=True)
        return
    state = {
        "status": "running",
        ...
        "unfreeze_mlp": True,
        "cnn_learning_rate": config["training"]["cnn_learning_rate"],
        "mlp_learning_rate": config["training"]["mlp_learning_rate"],
