"""Run the predeclared v3.3 global-spectral-attention ablation."""
...
CONFIG_PATH = (
    ...
    / "configs"
    / "experiments"
    / "sonicom_mlp_cnn_q26_v33_global_attention_e10.json"
)
OUTPUT_DIR = (
    ROOT
    / "artifacts"
...
def _command(config: dict) -> list[str]:
    ...
    return [
        ...
        "--global-attention-width",
        str(architecture["global_attention_width"]),
        "--global-attention-heads",
        str(architecture["global_attention_heads"]),
        "--global-attention-blocks",
        str(architecture["global_attention_blocks"]),
        "--global-attention-stride",
        str(architecture["global_attention_stride"]),
        "--freeze-local-cnn",
...
def main() -> None:
    ...
    if not config["training"]["freeze_local_cnn"]:
        raise RuntimeError("The v3.3 ablation must freeze the local CNN")
    ...
    if _complete(config):
        print("Completed v3.3 run found; skipping.", flush=True)
        return
    state = {
        "status": "running",
        ...
        "trainable_modules": ["global_context", "global_gate"],
