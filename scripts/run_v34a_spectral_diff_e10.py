"""Run the predeclared v3.4a high-frequency spectral-difference ablation."""
...
CONFIG_PATH = (
    ...
    / "configs"
    / "experiments"
    / "sonicom_mlp_cnn_q26_v34a_spectral_diff_e10.json"
)
OUTPUT_DIR = (
    ROOT
    / "artifacts"
...
def _command(config: dict) -> list[str]:
    ...
    return [
        ...
        str(objective["first_difference_weight"]),
        "--high-frequency-second-difference-weight",
        str(objective["second_difference_weight"]),
        "--spectral-difference-minimum-frequency-hz",
        str(objective["spectral_difference_minimum_frequency_hz"]),
        "--ild-weight",
...
def main() -> None:
    ...
    if config["training"]["high_frequency_weight"] != 0.25:
        raise RuntimeError("v3.4a must not increase the existing HF scalar weight")
    ...
    if _complete(config):
        print("Completed v3.4a run found; skipping.", flush=True)
        return
    state = {
        "status": "running",
        ...
        "first_difference_weight": config["objective"][
            "first_difference_weight"
        ],
        "second_difference_weight": config["objective"][
