"""Run the predeclared v3.4b horizontal spectral-band ILD ablation."""
...
CONFIG_PATH = (
    ...
    / "configs"
    / "experiments"
    / "sonicom_mlp_cnn_q26_v34b_band_ild_e10.json"
)
OUTPUT_DIR = (
    ROOT
    / "artifacts"
...
def _command(config: dict) -> list[str]:
    ...
    return [
        ...
        str(objective["minimum_band_center_hz"]),
        "--spectral-band-ild-maximum-center-hz",
        str(objective["maximum_band_center_hz"]),
        "--spectral-band-ild-beta-db",
        str(objective["smooth_l1_beta_db"]),
        "--ild-weight",
...
def main() -> None:
    ...
    if config["objective"]["spectral_difference_weights"] != [0.0, 0.0]:
        raise RuntimeError("v3.4b must isolate band ILD from spectral differences")
    if config["training"]["high_frequency_weight"] != 0.25:
        raise RuntimeError("v3.4b must not increase the existing HF scalar weight")
    ...
    if _complete(config):
        print("Completed v3.4b run found; skipping.", flush=True)
        return
    state = {
        "status": "running",
        ...
        "spectral_band_ild_weight": config["objective"][
            "spectral_band_ild_weight"
        ],
        "retained_band_count": config["objective"]["retained_band_count"],
