"""Predict a hash-locked formal Stage-E bounded-correction ensemble."""
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Stage-E ensemble validation inference")

    root = project_root()
    manifest_path = arguments.manifest.resolve()
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    identity = manifest_identity(manifest)
    if manifest["status"] != "frozen" or manifest["e_final"] != 25:
        raise ValueError("Manifest is not the frozen formal Stage-E E25 ensemble")
    if manifest["dataset"]["split"] != "val":
        raise PermissionError("Stage-E formal inference permits validation only")
